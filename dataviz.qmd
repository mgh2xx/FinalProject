...
for ax, season in zip(axs, [1, 2]):
    ...
    plot_c = sns.lineplot(data = rolling_df[(rolling_df["Season"] == season) & (rolling_df["About"] == "Athlete C")], x = "Date", y = "Prop_Jump_High", color = 'lightblue', ax = ax)
    ax.set_title(f"Season {season}", weight = 'bold')
    ax.set_ylabel('')# Proportion of High-Band Jumps
    ax.legend_.remove()
    sns.despine(ax=ax, top=True, right=True, left=False, bottom=False)
    ax.tick_params(axis='both', which='both', length=0, width=0)

# Showing plots
fig.text(0.29, 0.95, 'High-Band Jump Trends by Player: Comparing', fontsize=20, rotation='horizontal', weight='bold')
fig.text(0.53, 0.95, 'One-Season', fontsize=20, rotation='horizontal', weight='bold', color='orange')
fig.text(0.594, 0.95, 'vs', fontsize=20, rotation='horizontal', weight='bold')
fig.text(0.608, 0.95, 'Two-Season', fontsize=20, rotation='horizontal', weight='bold', color='navy')
fig.text(0.6715, 0.95, 'Tenure', fontsize=20, rotation='horizontal', weight='bold')
fig.text(0.11, 0.89, 'Proportion', fontsize=10, rotation='horizontal', weight='normal')
plt.subplots_adjust(wspace=0.05)  # Decrease this value to move subplots closer
...
plt.suptitle('Athlete C Sees Significant Change in Jump-Band Proportions Over the Course of 2 Seasons'
             , x = 0.33, y = 0.965, weight = "bold")
